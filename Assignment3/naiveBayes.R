...
naive.bayes <- function(data.training, column.class, data.testing) {
  ...
  for(class in classes) { 
    ...
    for(variable in variables) {
      ...
      for(possible.value in possible.values) {
        ...
        rowIdx <- paste(variable, possible.value, sep="_");
  ...
  for(i in 1:nrow(data.testing)) {
    ...
    for(class in classes) {
      ...
      for(variable in variables) {
        prob <- prob * prob.table[paste(variable, data.testing[i, variable], sep="_"), classIdx];
      }
...
naivebayes.testing.results <- naive.bayes(naivebayes.training, 'Class', naivebayes.testing);

# Print out the number of correct predictions
numCorrect <- sum(naivebayes.testing.results$Class == naivebayes.testing.results$Prediction);
print(paste("Accuracy: ", ( numCorrect * 100 ) / nrow(naivebayes.testing.results), "%", sep=""));

# Same confusion matrix as e1071.R
table(naivebayes.testing.results$Prediction, naivebayes.testing$Class)