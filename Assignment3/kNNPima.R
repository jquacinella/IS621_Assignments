...
k <- 3;
nearestNeighbor.public.results <- nearestNeighbor(nearestNeighbor.training, 'class', k, nearestNeighbor.public);

# Print out the number of correct predictions
numCorrect <- sum(nearestNeighbor.public.results$class == nearestNeighbor.public.results$Prediction);
print(paste("Accuracy: ", ( numCorrect * 100 ) / nrow(nearestNeighbor.public.results), "%", sep=""));

# Confusion Matrix
table(nearestNeighbor.public.results$Prediction, nearestNeighbor.public.results$class);